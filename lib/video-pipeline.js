...
export async function extractFromVideoUrl({ url, analysis = 'auto', onProgress = () => {}, prefetchedReel = null }) {
  ...
  if (platform === 'youtube') {
    ...
    const transcript = await fetchYouTubeTranscript(url).catch(err => {
...
// ---------------------------------------------------------------------------
// YouTube transcript fetcher — used by the YouTube branch since
// googlevideo.com blocks our datacenter IPs and Gemini-on-video isn't
// reachable. Scrapes the watch page for ytInitialPlayerResponse, picks
// an English caption track, fetches the timed-text XML, and returns
// plain text. Returns null when:
//   - URL has no recognizable video ID
//   - watch page fetch is non-200 (consent / bot challenge)
//   - the player response has no caption tracks (no captions on video)
...
async function fetchYouTubeTranscript(url) {
  ...
  console.log('[youtube-transcript] videoId:', videoId);
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`[youtube-transcript] watch page status ${res.status}`);
      return null;
    }
    html = await res.text();
  } catch (e) {
    console.warn('[youtube-transcript] watch page fetch failed:', e.message);
    return null;
  }

  const tracksJson = extractCaptionTracksJson(html);
  if (!tracksJson) {
    console.warn('[youtube-transcript] captionTracks not found in watch page (no captions, or bot challenge)');
    return null;
  }
  ...
  try { tracks = JSON.parse(tracksJson); }
    ...
    console.warn('[youtube-transcript] captionTracks JSON parse failed:', e.message);
    return null;
  }
  if (!Array.isArray(tracks) || !tracks.length) return null;

  // Prefer English (manual or auto), then anything else.
  const track =
  ...
  // YouTube's serialized JSON often unicode-escapes ampersands.
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`[youtube-transcript] caption fetch status ${res.status}`);
      return null;
    }
    xml = await res.text();
  } catch (e) {
    console.warn('[youtube-transcript] caption fetch failed:', e.message);
    return null;
  }
  ...
  console.log(`[youtube-transcript] got ${transcript.length} chars from ${lines.length} cues`);
...
// Locate the captionTracks JSON array inside the watch page. Bracket-
// counting + string-skipping keeps URLs containing ']' from breaking
// the match (a non-greedy regex would).
